...
export async function GET(request) {
    try {
        ...
        const summary = {
            ...
            warehouseStockProducts: normalizedProducts.filter((product) => Number(product.inventory?.warehouse || 0) > 0).length,
            kitchenStockProducts: normalizedProducts.filter((product) => Number(product.inventory?.kitchen || 0) > 0).length,
            loungeStockProducts: normalizedProducts.filter((product) => Number(product.inventory?.lounge || 0) > 0).length,
